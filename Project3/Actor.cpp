...
void Actor::changeCoords(int r, int c){
    m_row = r;
    m_col = c;
}
...
void Actor::move(char direction){
    switch(direction){
        case ARROW_UP:
            if(row() > 0 && getLevel()->validMove(row()-1, col())){
...
void Actor::attack(int r, int c){
    bool hitBool = false;
    //bool isSleep = false;
    Actor* player = getLevel()->player();
    if(getLevel()->player()->row() == r && getLevel()->player()->col() == c){
        int monsterStats = dex() + getWeapon()->dex();
        int playerStats = player->dex() + player->armor();
        
        int randPlayerStats = randInt(1,playerStats);
        int randMonsterStats = randInt(1,monsterStats);
        
        if(randInt(1,randMonsterStats) >= randInt(1,randPlayerStats)){
            hitBool = true;
            player->changeHP(-1*randInt(0,str()+getWeapon()->damage()-1));
        }
    }
    
    if(hitBool){
        if(player->isDead()){
            std::cout << name() << " " << getWeapon()->action() << " " << getWeapon()->name() << " at the " << player->name() << " dealing a final blow." << std::endl;
            return;
        }
        //put sleep shit
        else{
            std::cout << name() << " " << getWeapon()->action() << " " << getWeapon()->name() << " at the " << player->name() << " and hits." << std::endl;
        }
    }
    else{
        std::cout << name() << " " << getWeapon()->action() << " " << getWeapon()->name() << " at the " << player->name() << " and misses." << std::endl;
        return;
    }
}
        ...
        switch (pos) {
            ...
            case 'G':
                ...
                if(randPlayerStats >= randMonsterStats){
                    hitBool = true;
                    std::cout << getWeapon()->damage() << std::endl;
                    mob->changeHP(-1*randInt(0,str() + getWeapon()->damage()-1));
                }
                if(hitBool){
                    if(mob->isDead()){
                        std::cout << name() << " " << getWeapon()->action() << " " << getWeapon()->name() << " at the " << mob->name() << " dealing a final blow." << std::endl;
                        getLevel()->killMonsters();
                        return;
                    }
                    //put sleep shit
                    else{
                        std::cout << name() << " " << getWeapon()->action() << " " << getWeapon()->name() << " at the " << mob->name() << " and hits." << std::endl;
                    }
                }
                else{
                    std::cout << name() << " " << getWeapon()->action() << " " << getWeapon()->name() << " at the " << mob->name() << " and misses." << std::endl;
                    return;
                }
        }
    }
